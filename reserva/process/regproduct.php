    ...
    if($codeProd!="" && $nameProd!="" && $cateProd!="" && $priceProd!="" && $modelProd!="" && $marcaProd!="" && $stockProd!="" && $codePProd!=""){
        ...
        if($verificaltotal<=0){
            if($imgType=="image/jpeg" || $imgType=="image/png"){
                        ...
                        if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, Precio, Descuento, Modelo, Marca, Stock, NITProveedor, Imagen, Nombre, Estado", "'$codeProd','$nameProd','$cateProd','$priceProd', '$descProd', '$modelProd','$marcaProd','$stockProd','$codePProd','$imgFinalName','$adminProd', '$estadoProd'")){
                            echo '<script>
                                swal({
                                  ...
                                  text: "El SERVICIO se añadió a DREAMZONE con éxito",
                                  type: "success",
                ...
                echo '<script>swal("ERROR", "El formato de la imagen del SERVICIO es invalido, solo se admiten archivos con la extensión .jpg y .png ", "error");</script>';
            }
        }else{
            echo '<script>swal("ERROR", "El código de SERVICIO que acaba de ingresar ya está registrado en el sistema, por favor ingrese otro código de SERVICIO distinto", "error");</script>';
        }
    }else {
